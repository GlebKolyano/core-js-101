...
function getRectangleString(width, height) {
  ...
  const repeatChar = (char, n) => char.repeat(n);
  const top = `┌${repeatChar('─', w)}┐\n`;
  const sides = repeatChar(`│${repeatChar(' ', w)}│\n`, h);
  const bottom = `└${repeatChar('─', w)}┘\n`;
...
function encodeToRot13(str) {
  ...
  return str
    ...
    .map((char) => {
      if (isLetter(char)) {
        const index = decode.indexOf(char);
        return encode[index];
      }
      return char;
    })
